...
export default function HomePage() {
  ...
  const [loadedCount, setLoadedCount] = useState(5); // infinite scroll simulation
  ...
  return (
    <main style={{ maxWidth: "600px", margin: "2rem auto", fontFamily: "Arial, sans-serif" }}>
      <header style={{ textAlign: "center", marginBottom: "2rem" }}>
        <h1 style={{ fontSize: "2rem", marginBottom: "0.25rem" }}>Social Feed</h1>
      ...
      <section style={{ marginBottom: "2rem" }}>
        <textarea
          ...
          style={{ width: "100%", padding: "0.75rem", borderRadius: "10px", border: "1px solid #ccc", resize: "none", fontSize: "1rem" }}
          rows={3}
        />
        <button
          onClick={addPost}
          style={{ marginTop: "0.5rem", padding: "0.5rem 1.25rem", borderRadius: "10px", border: "none", backgroundColor: "#0070f3", color: "#fff", cursor: "pointer", fontWeight: "bold" }}
      ...
      <section style={{ maxHeight: "75vh", overflowY: "auto" }}>
        ...
        {loadedCount < posts.length && (
          <button
            onClick={loadMore}
            style={{ width: "100%", padding: "0.5rem", marginTop: "0.5rem", borderRadius: "8px", border: "1px solid #0070f3", backgroundColor: "#fff", color: "#0070f3", cursor: "pointer", fontWeight: "bold" }}
...
function PostCard({ post, likePost, addComment, editPost, deletePost, formatDate }: { post: Post; likePost: (id: number) => void; addComment: (postId: number, content: string) => void; editPost: (id: number, content: string) => void; deletePost: (id: number) => void; formatDate: (date: Date) => string }) {
  const [editing, setEditing] = useState(false);
  const [editContent, setEditContent] = useState(post.content);

  return (
    <div style={{ display: "flex", flexDirection: "column", border: `2px solid ${post.color}`, borderRadius: "12px", padding: "1rem", marginBottom: "1rem", backgroundColor: "#fff", boxShadow: "0 2px 6px rgba(0,0,0,0.05)" }}>
      ...
      {editing ? (
        <div style={{ marginBottom: "0.5rem" }}>
          <textarea value={editContent} onChange={e => setEditContent(e.target.value)} style={{ width: "100%", borderRadius: "8px", padding: "0.5rem", fontSize: "0.95rem" }} rows={2} />
      ...
      <div style={{ display: "flex", alignItems: "center", marginBottom: "0.5rem" }}>
        <button onClick={() => likePost(post.id)} style={{ padding: "0.3rem 0.6rem", borderRadius: "6px", border: `1px solid ${post.color}`, backgroundColor: "#fff", color: post.color, cursor: "pointer", fontSize: "0.9rem", fontWeight: "bold", marginRight: "0.5rem" }}
...
function CommentInput({ postId, addComment }: { postId: number; addComment: (postId: number, content: string) => void }) {
  ...
  return (
    <div style={{ display: "flex", marginTop: "0.5rem" }}>
      <input type="text" placeholder="Write a comment..." value={comment} onChange={e => setComment(e.target.value)} style={{ flex: 1, padding: "0.5rem", borderRadius: "8px", border: "1px solid #ccc", fontSize: "0.9rem" }} />
      <button onClick={() => { addComment(postId, comment); setComment(""); }} style={{ marginLeft: "0.5rem", padding: "0.4rem 0.8rem", borderRadius: "8px", border: "none", backgroundColor: "#0070f3", color: "#fff", cursor: "pointer", fontWeight: "bold" }}>Comment</button>
    </div>
  );
}      timestamp: new Date(Date.now() - i * 60000),
      comments: [],
